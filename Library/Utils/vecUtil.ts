...
{
    return new Vector3(u.getY*v.getZ-u.getZ*v.getY, u.getZ*v.getX-u.getX*v.getY, u.getX*v.getY-u.getY*v.getX);
}
...
{
    return new Vector3(randomBetween(min,max),randomBetween(min,max),randomBetween(min,max));
}
...
export function refract(uv: Vector3, n: Vector3, etai_over_etat: number): Vector3
{
