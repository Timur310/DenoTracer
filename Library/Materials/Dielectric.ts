...
export class Dielectric implements Material
{
    ...
    scatter(r_in: Ray): boolean
    {
        ...
        const refractionRatio = Record.Instance.front_face ? (1.0/this.refractionIndex) : this.refractionIndex;
        const unitDir = unitVector(r_in.getDirection);
        
        const cos_theta = Math.min(dot(unitDir.negate(),Record.Instance.normal),1.0);
        ...
        if(cannotRefract || this.reflectance(cos_theta, refractionRatio) > random())
        {
