...
function randomScene(): HitableList
{
    ...
    for(let a=-11;a<11;a++)
    {
        ...
        {
            const mat = random();
            const center = new Point(a+0.9*random(),-0.3,b+0.9*random());

            if(substractVector(center,new Point(4,0,0)).sqrtLength()>0.9)
            {
                if(mat<0.8)
                {
                ...
                {
                    const albedo = new Color(randomBetween(0.5,1),randomBetween(0.5,1),randomBetween(0.5,1));
