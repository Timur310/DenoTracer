...
export class bvh_node implements bvh_tree<Hittable> {
	...
	constructor(objects: HitableList, start: number, end: number) {
		...
		const comparator = (axis === 0)
			? (a: Hittable, b: Hittable) => this.box_x_compare(a, b)
			: (axis === 1)
			? (a: Hittable, b: Hittable) => this.box_y_compare(a, b)
			: (a: Hittable, b: Hittable) => this.box_z_compare(a, b);
		...
		if (!this.left.bounding_box() || this.right.bounding_box()) {
