...
import { addVector, cross, divideVectorN, multiplyVectorN, randomInUnitDisk, unitVector } from "../Utils/vecUtil.ts";
...
export class Camera
{
    ...
    constructor(lookFrom: Point, lookAt: Point, vUp: Vector3, aperture: number,focusDist: number, fov: number,aspect_ratio: number)
    {
        ...
        this.horizontal = multiplyVectorN(this.u,this.viewport_width*focusDist);
        this.vertical = multiplyVectorN(this.v,this.viewport_height*this.focusDist);
        this.lower_left_corner = substractVector(this.origin,divideVectorN(this.horizontal,2)).substract(divideVectorN(this.vertical,2)).substract(multiplyVectorN(this.w,this.focusDist));
    ...
    getRay(u: number, v: number): Ray
    {
        ...
        const offset = multiplyVectorN(this.u,rd.getX).add(multiplyVectorN(this.v,rd.getY));

        const a1 = multiplyVectorN(this.horizontal,u);
        const a2 = multiplyVectorN(this.vertical,v);

        return new Ray(addVector(this.origin,offset), addVector(this.lower_left_corner,a1).add(a2).substract(this.origin).substract(offset));
    }
}